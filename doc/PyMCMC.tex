...
code is as follows:
\begin{itemize}
\item Lines 3-7 import the classes and functions that are required in
  this example.
\item Line 11 is used to obtain the path of the data if installed
  using setup.py. Note that this step will not be necessary for normal
  usage of \pkg{PyMCMC}.
\item Lines 13-15 define the function that is used to sample
  $\bm{\gamma}.$
\item The main program starts at line 18.
\item Lines 21-24 are used to load the data, from which arrays
  $\bm{y}$ (\code{yvec}) and $\bm{X}$ (\code{xmat}) are constructed.
\item Lines 28-31 construct the dictionary \code{data}, which includes
  \code{yvec, xmat} and the class instance of \code{StochasticSearch}.
\item Line 36 defines the class instance \code{simgam}, which defines
  the block of the MCMC scheme to sample $\bm{\gamma}$.
\item Line 39 defines the class instance of \code{MCMC}. Note that the
...
\item Line 41 produces the standard output for the MCMC sampler, which
  is saved in the file \code{`vs.txt'}.
\item Line 42 produces the custom output from the stochastic search
  class and is sqaved in the file \code{'SSVS.out'}.
\item Line 43 proqduces the custom output for the stochastic search
  class and writes it to the screen.
\item Lines 45-48 produces regression output, from a Bayesian
  regression analysis based on the most probable model.
\item Line 49 produces standard plots from the Bayesian regression
  analysis.
\end{itemize}
...
\item Lines 16-18 defines a function, \code{minfunc}, which is
...
\item Line 62 initialises the class \code{RWMH}. 
\item Line 63 initialises the class \code{MCMC} . Note that the
...
\item Line 97 initialises the class instance \code{simsigbeta}, which is
...
\item Line 107 initialises \code{MCMC}. The MCMC sampler is run for 8000
