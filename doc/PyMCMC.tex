...
slice sampler. The user can simply piece together the algorithms
...
$\gamma$. The single argument to \code{samplegamma}, \code{store}, is
a dictionary (\proglang{Python} data structure) that is passed to all
functions that are called from the MCMC sampler.  The purpose of
\code{store} is to contain all the data required to define functions
that sample from, or are used in the evaluation of, the posterior
distribution. In this example, \code{store} is defined to contain the
observational vector, $\bm{y}$, the matrix of regressors $\bm{X}$ and
the class instance for \code{StochasticSearch}:
...
Notice that in this example we use Zellner's g-prior with
...
of closed form, we can use the \code{CFsampler} class, called with our
\code{samplegmma} function defined earlier, initial values and a
parameter name \code{gamma}. The initial value for $\gamma$ is
$(1,0,\ldots,0)$. 
...
By default, each sampler in PyMCMC will add a dictionary called
\code{store}, which stores the parameter value at each iteration. This
means that the user can access the current iteration by calling
\code{store{[}`gamma'{]}} ***CHECK****. In this example, we turn this
feature off by setting \code{store = `none'}.
...
iterations (2000), the burn in (500), providing the data dictionary
...
of variable 12. The marginal posterior densities (Figure
\ref{Flo:mpdreg}) can be created using
\begin{Code}
breg.plot()
\end{Code}
...
$\hat{\beta}_{1}$ = 0.157 {[}0.148, 0.165{]}. The marginal posterior
densities of these estimates (Figure 2) confirm that both estimates
are far from zero.

The ACF plots (Figure \ref{fig:plot_output_eg2}) and low inefficiency
factors (see \citet{ChibGreenberg1996} for details on inefficiency
factors) of 13.5 ($\hat{\beta}_{0}$) and 12.2 ($\hat{\beta}_{1}$) show
that autocorrelation in the sample is relatively low for an MCMC
sampler. %
\begin{figure}[t!]
  \begin{center}
...
\caption{Marginal posterior density, autocorrelation function and
  ...
  are calculated after removing the burn in.}
...
subprograms (\pkg{BLAS}) and preferable also the automatically
tuned
...
calculation of $\bm{X}\bm{\beta}.$ It is stored in the
...
passed to the function \code{loglinear.logl} then \pkg{F2py} will
...
that \pkg{F2py} and \pkg{ATLAS} are installed correctly, save the
following code as, for example, \code{blas\_eg.f90}:
